...
# SE-module: Squeeze excitation module
...
# Residual block (conv & skip connections)
...
# Channel attention block (Original is CBAM module)
...
# CBAM (Convolutional block attention module) for Channel & spatial attention block
...
# ResBlock which contain CBAM attention
...
# Filtering with Laplacian module
...
class LaplacianPyramid(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = F.interpolate(x, scale_factor=0.125, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        x1 = F.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        x2 = F.interpolate(x, scale_factor=0.5, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        ...
        lap_0 = F.interpolate(lap_0, scale_factor=8, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        lap_1 = F.interpolate(lap_1, scale_factor=4, mode='bilinear', align_corners=False, recompute_scale_factor=True)
        lap_2 = F.interpolate(lap_2, scale_factor=2, mode='bilinear', align_corners=False, recompute_scale_factor=True)

        return torch.cat([lap_0, lap_1, lap_2, lap_3], 1)
### end class

# Main network class
# LRM: Location-aware reflection removal network
class LRM(nn.Module):
